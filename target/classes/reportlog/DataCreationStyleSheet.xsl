...
<xsl:template match="root">
	<html>
	<head>
	<title>Data Automation Manager</title>
...
<xsl:template match="test-scenario">
	<div class="box" id="">
		<div class="header1"><span></span>Test Data: <xsl:value-of select="@name"/></div>
		<xsl:apply-templates select="scen-coverage"/>	
		<xsl:apply-templates select="profile-data"/>
		<xsl:apply-templates select="biller-data"/>
	</div>
</xsl:template>

<!--Template for Summary/Details--> 
<xsl:template match="scen-coverage">	
		<div class="header4"><span class="label">Scenario Coverage: </span><span><xsl:value-of select="@name" /></span></div>	
</xsl:template>

<!--Template for Test Cases--> 
<xsl:template match="profile-data">
		<div class="header2"><span class="label">Profile: </span><xsl:value-of select="@name"/></div>
		<div class="header3" id="profile">			
			<xsl:apply-templates select="profile-AG"/>
			<xsl:apply-templates select="profile-login"/>			
			<xsl:apply-templates select="profile-liability"/>
			<xsl:apply-templates select="profile-FAN"/>
		</div>
	
</xsl:template>

<!--Template for Test Cases--> 
<xsl:template match="biller-data">	
	<div class="header2"><span></span>Biller: <xsl:value-of select="@name"/></div>
	<div class="header3" id="biller">						
	<xsl:apply-templates select="biller-BAN"/>
	<xsl:apply-templates select="biller-CTN"/>
	</div>
</xsl:template>

 <!--Template for Test Steps--> 
<xsl:template match="profile-login">	
		<div><span>Login: </span><xsl:value-of select="@name"/>
	</div>
</xsl:template>

 <!--Template for Test Steps--> 
<xsl:template match="profile-AG">
		<div><span>Account Group: </span><xsl:value-of select="@name"/>
	</div>
</xsl:template>

<!--Template for Test Steps--> 
<xsl:template match="profile-liability">
		<div><span>Liability: </span><xsl:value-of select="@name"/>
	</div>
</xsl:template>

<!--Template for Test Steps--> 
<xsl:template match="profile-FAN">
		<div><span>FAN: </span><xsl:value-of select="@name"/>
	</div>
</xsl:template>

 <!--Template for Test Steps--> 
<xsl:template match="biller-BAN">
		<div><span>BAN: </span><xsl:value-of select="@name"/>
	</div>
</xsl:template>

<!--Template for Test Steps--> 
<xsl:template match="biller-CTN">	
		<div><span>CTN: </span><xsl:value-of select="@name"/>
	</div>
</xsl:template>
